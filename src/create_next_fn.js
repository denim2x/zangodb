...
const getIDBReqWithIndex = (store, clause) => {
  const key_range = clause.idb_key_range || null,
    ...
    if (literal === '_id') { index = store } else { index = store.index(literal) }
...
const initPredAndSortSpec = (config) => {
  const { pipeline } = config,
    ...
    for (let [fn, arg] of pipeline) {
      if (fn === sort) { sort_specs.push(arg) } else if (fn === filter) { preds.push(arg) } else { break }
  ...
  pipeline.splice(0, i)
    ...
    if (sort_specs.length) {
      config.sort_spec = sort_specs[0]
        if (sort_specs.length > 1) {
          console.warn('Only one sort allowed per person')
        }
    }
}
...
const createParallelNextFn = (config) => {
  ...
  const _id_hashes = new Set()
    ...
    const changeNextFn = (cb) => {
      if (currentNextFn = getNextFn()) { next(cb) } else { cb() }
    }

    const next = (cb) => {
      currentNextFn((error, doc, idb_cur) => {
        if (error) { cb(error) } else if (!doc) { changeNextFn(cb) } else if (onDoc(doc)) {
          cb(null, doc, idb_cur)
            } else { next(cb) }
      })
    };
  ...
  const getIDBCur = createGetIDBCurFn(config)
    ...
    const next = (cb) => {
      getIDBCur((error, idb_cur) => {
        if (!idb_cur) { cb(error) } else { cb(null, idb_cur.value, idb_cur) }
      })
    };
...
module.exports = (cur) => {
  let pipeline
    ...
    try { pipeline = buildPredicates(cur._pipeline) } catch (error) { return cb => cb(error) }
